...
class DelistedDaysMixin(BaseSync):
    ...
    混入计算退市日的功能
    """
    def _gen_delisted_info(self, code, timestamp):
        """
    ...
    def _gen_delisted_days(self, infos, limit_date):
        ...
        for j in range(len(infos)-1):
            ...
            if start == end:
                ...
                delisted_days.extend(self.get_date_list(start.get("change_date"), end.get("change_date")))
        ...
        if last_info.get("is_listed"):
            ...
            delisted_days.extend(self.get_date_list(last_info.get("change_date"), limit_date))
    ...
    def delisted_days(self, code, timestamp, limit_date):
        infos = self._gen_delisted_info(code, timestamp)
        delisted_days = self._gen_delisted_days(infos, limit_date)
        return delisted_days
